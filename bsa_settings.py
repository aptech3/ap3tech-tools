...
def get_all_merchants_with_ids():
    """Return list of (id, name, co, address, city, state, zip, notes) for all merchants."""
    conn = connect_db()
    c = conn.cursor()
    c.execute("SELECT id, name, co, address, city, state, zip, notes FROM MerchantProcessors ORDER BY name COLLATE NOCASE")
    result = c.fetchall()
    conn.close()
    return result
...
def get_merchant_by_id(row_id):
    ...
    conn = connect_db()
    c = conn.cursor()
    c.execute("SELECT id, name, co, address, city, state, zip, notes FROM MerchantProcessors WHERE id = ?", (row_id,))
    row = c.fetchone()
    conn.close()
    if row:
        keys = ["id", "name", "co", "address", "city", "state", "zip", "notes"]
        return dict(zip(keys, row))
    else:
        return None

def add_merchant_full(name, co="", address="", city="", state="", zip_code="", notes=""):
    ...
    try:
        c.execute("""
            INSERT INTO MerchantProcessors 
                (name, co, address, city, state, zip, notes, date_added) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
            (name.strip(), co.strip(), address.strip(), city.strip(), state.strip(), zip_code.strip(), notes.strip(), datetime.now().isoformat()))
        conn.commit()
    except sqlite3.IntegrityError:
        pass
    conn.close()

def edit_merchant_by_id(row_id, name, co, address, city, state, zip_code, notes):
    """Edit a merchant by row ID, updating all fields."""
    conn = connect_db()
    c = conn.cursor()
    c.execute("""
        UPDATE MerchantProcessors 
        SET name = ?, co = ?, address = ?, city = ?, state = ?, zip = ?, notes = ?
        WHERE id = ?""",
        (name.strip(), co.strip(), address.strip(), city.strip(), state.strip(), zip_code.strip(), notes.strip(), row_id))
    conn.commit()
    conn.close()
...
def add_suggestion(name, found_in_file=""):
    ...
    conn = connect_db()
    c = conn.cursor()
    c.execute("INSERT INTO Suggestions (name, date_found, found_in_file) VALUES (?, ?, ?)", (name, datetime.now().isoformat(), found_in_file))
    conn.commit()
    conn.close()
...
def export_merchants_txt(filepath):
    ...
    with open(filepath, "w", newline='', encoding="utf-8") as f:
        writer = csv.writer(f)
        # Write header
        writer.writerow(["name", "co", "address", "city", "state", "zip", "notes"])
        for _, name, co, address, city, state, zip_code, notes in merchants:
            writer.writerow([name, co, address, city, state, zip_code, notes])
...
def import_merchants_txt(filepath):
    ...
    with open(filepath, "r", encoding="utf-8") as f:
        ...
        first = True
        for row in reader:
            if first and row and row[0].lower() == "name":
                first = False  # Skip header
                continue
            if len(row) < 1: continue
            # Safely unpack all columns (pad with empty if missing)
            fields = (row + [""] * 7)[:7]
