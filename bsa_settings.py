...
def get_all_merchants_with_ids():
    ...
    conn = connect_db()
    c = conn.cursor()
    c.execute("SELECT id, root, name, co, address, city, state, zip, notes FROM MerchantProcessors ORDER BY name COLLATE NOCASE")
    result = c.fetchall()
    conn.close()
    return result
...
def get_merchant_by_id(row_id):
    ...
    conn = connect_db()
    c = conn.cursor()
    c.execute("SELECT id, root, name, co, address, city, state, zip, notes FROM MerchantProcessors WHERE id = ?", (row_id,))
    row = c.fetchone()
    conn.close()
    if row:
...
def add_merchant_full(root, name, co="", address="", city="", state="", zip_code="", notes=""):
    ...
    try:
        c.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (root.strip(), name.strip(), co.strip(), address.strip(), city.strip(), state.strip(), zip_code.strip(), notes.strip(), datetime.now().isoformat()))
        conn.commit()
...
def edit_merchant_by_id(row_id, root, name, co, address, city, state, zip_code, notes):
    ...
    c.execute("""
        ...
        WHERE id = ?""",
        (root.strip(), name.strip(), co.strip(), address.strip(), city.strip(), state.strip(), zip_code.strip(), notes.strip(), row_id))
    conn.commit()
    conn.close()
...
def import_merchants_txt(filepath):
    ...
    with open(filepath, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if first and row and (row[0].lower() in ["root", "name"]):  # supports files with/without root
                first = False  # Skip header
                continue
            if len(row) < 2: continue
            fields = (row + [""] * 8)[:8]  # 8 fields: root, name, co, address, city, state, zip, notes
